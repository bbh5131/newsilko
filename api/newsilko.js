...
// 5) ✅ 기사별 og:image 수집 제거: newsilko 고정 브랜드 썸네일 사용
// public/newsilko-thumbnail.png 파일을 배포하면 아래 URL로 바로 표시됨
const THUMBNAIL_URL =
...
async function callOpenAI(
  messages,
  { temperature = 0.4, max_tokens = 220, timeoutMs = 2200 } = {}
) {
...
JSON 형식:
{
  ...
  const r = await callOpenAI(
    ...
    { temperature: 0.2, max_tokens: 260, timeoutMs: 1200 }
  );
...
- 출력에 성 포함 원문 인명이 다시 등장하면 안 됨`;
  ...
  return await callOpenAI(
    ...
    { temperature: 0.95, max_tokens: 170, timeoutMs: 2200 }
  );
}
...
async function makeCasual(title) {
  ...
  // 카카오 응답 지연을 막기 위해 추가 GPT 재시도는 하지 않는다.
  // 첫 변환 결과가 제목과 비슷해도 우선 즉시 반환한다.
  return {
    ok: true,
    text: a.text,
    why: "",
...
export default async function handler(req, res) {
  try {
    ...
    // 기사 이미지 HTML fetch 없이 GPT만 처리한다.
    // 썸네일은 항상 우리 Vercel의 고정 브랜드 이미지를 사용한다.
